...
data1990<- read.csv("nhgis0005_ds123_1990_tract.csv") %>%
          filter(STATE == "Michigan" & E4U001 > 0)%>%
          mutate(MedianHHIncome1990 = E4U001)%>%
          select(GISJOIN,MedianHHIncome1990)
...
tracts1990 <-sf::st_read(dsn = "nhgis0005_shapefile_tl2008_us_tract_1990",
...
saveRDS(tracts1990, file="tracts1990.RDS")
...
leaflet(tracts1990) %>%
  ...
  addLegend(pal = pal, values = ~MedianHHIncome1990, opacity = 1.0)
